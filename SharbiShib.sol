...
contract SHARBI_Token is IERC20Extended, Ownable {
    ...
    constructor() Ownable() {
        ...
        marketFeeReceiver = msg.sender;
        burnFeeReceiver = address(0xdead);
        ...
        isLimitExmpt[burnFeeReceiver] = true;
        isLimitExmpt[marketFeeReceiver] = true;
        isLimitExmpt[address(this)] = true;
        isLimitExmpt[address(router)] = true;

        isWalletExmpt[autoLiquidityReceiver] = true;
        isWalletExmpt[burnFeeReceiver] = true;
        isWalletExmpt[marketFeeReceiver] = true;
    ...
    function setBuyAccFee(uint256 _amount) internal {
        ...
        _buybackShibFeeCount += _amount.mul(_buybackShibFee).div(
            feeDenominator
        );
    }
    ...
    function setSellAccFee(uint256 _amount) internal {
        ...
        _marketFeeCount += _amount.mul(_marketSellFee).div(feeDenominator);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBBuyback = amountBNB.mul(_buybackShibFeeCount).div(
            totalBNBFee
        );
        if (amountBNBBuyback > 0) {
            address[] memory path2 = new address[](2);
            path2[0] = router.WETH();
            path2[1] = address(SHIB);

            router.swapExactETHForTokensSupportingFeeOnTransferTokens{
                value: amountBNBBuyback
            }(0, path2, burnFeeReceiver, block.timestamp);
        }
        uint256 amountBNBMarket = amountBNB.mul(_marketFeeCount).div(
            totalBNBFee
        );
        if (amountBNBMarket > 0) {
            payable(marketFeeReceiver).transfer(amountBNBMarket);
        }

        _liquidityFeeCount = 0;
        _reflectionFeeCount = 0;
        _buybackShibFeeCount = 0;
        _marketFeeCount = 0;
    }
    ...
    function setBuyFees(
        ...
        _buybackShibFee = _buybackFee;
        totalBuyFee = _liquidityFee.add(_reflectionFee).add(_buybackFee);
        feeDenominator = _feeDenominator;
        require(
            totalBuyFee <= feeDenominator.mul(20).div(100),
    ...
    function setSellFees(
        ...
        totalSellFee = _liquidityFee.add(_reflectionFee).add(_marketFee);
        feeDenominator = _feeDenominator;
        require(
            totalBuyFee <= feeDenominator.mul(20).div(100),
            "Can't be greater than 20%"
